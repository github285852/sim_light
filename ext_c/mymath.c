...
float TriangleArea(COORD A,COORD B,COORD C)
{
	float result;
	result = (A.x*B.y + B.x*C.y + C.x*A.y - A.y*B.x - B.y*C.x - C.y*A.x)/2.0;
	if(result < 0)
		result = -1 * result;
	return result;
}

int InTriangle(COORD point,COORD A,COORD B,COORD C)
{
	float triangle_area = TriangleArea(A,B,C);
	float area;
	float erro;
	area = TriangleArea(point,A,B);
	area += TriangleArea(point,A,C);
	area += TriangleArea(point,B,C);
	erro = ABS(area-triangle_area);
	if(erro < 0.0001)
		return 1;
	else
		return 0;
